...
def celu_compute(x, y, alpha=0, kernel_name="celu"):
    ...
    if alpha == 0:
        ...
        zero_tensor = te.lang.cce.broadcast(tvm.const(0, inp_dtype), shape)
        alpha_tensor = te.lang.cce.broadcast(tvm.const(alpha, inp_dtype), shape)
        res_divide = te.lang.cce.vdiv(x, alpha_tensor)
        res_exp = te.lang.cce.vsub(te.lang.cce.vexp(res_divide), te.lang.cce.broadcast(tvm.const(1.0, inp_dtype), shape))
        res_mul = te.lang.cce.vmuls(res_exp, alpha)
        res_temp1 = te.lang.cce.vmax(x, zero_tensor)
        res_temp2 = te.lang.cce.vmin(x, res_mul)
